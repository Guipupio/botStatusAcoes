...
def completaExcel(browser, sht, sht2=None):
    ...
    data = datetime.now()
    sht.cells(1, coluna).value = str(data.day) + '/' + \
    ...
    while linha <= limit:
        try:
            ...
            browser.switch_to.frame(browser.find_elements_by_tag_name(
                'iframe')[0].get_attribute('id'))
            ...
            elementos.append(browser.execute_script(
                '''return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr.ticker.active.quote-ticker-inited > td.symbol-short-name-container').textContent'''))
            elementos.append(browser.execute_script(
                '''return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr.ticker.active.quote-ticker-inited > td.symbol-last').textContent'''))
            elementos.append(browser.execute_script(
                '''return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr.ticker.active.quote-ticker-inited > td.symbol-change').textContent'''))
            ...
            while True:
                elementos = []
                elementos.append(browser.execute_script("return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr:nth-child("+str(
                    linhaTabela) + ") > td.symbol-short-name-container').textContent"))
                elementos.append(browser.execute_script(
                    "return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr:nth-child("+str(linhaTabela) + ") > td.symbol-last').textContent"))
                elementos.append(browser.execute_script(
                    "return document.querySelector('#miniwidget > div.pages > div > table > tbody > tr:nth-child("+str(linhaTabela) + ") > td.symbol-change').textContent"))
    ...
    while linha <= limit:
        try:
            ...
            while True:
                ...
                sht.cells(linha, 'A').add_hyperlink(
                    sht.cells(linha + 1, 'A').hyperlink, sht.cells(linha + 1, 'A').value)
                num_add += 1
                limit += 1
    ...
    try:
        while True:
            item = browser.find_element_by_xpath(
                '//*[@id="ctl00_contentPlaceHolderConteudo_BuscaNomeEmpresa1_grdEmpresa_ctl01"]/tbody/tr[' + str(linha) + ']/td[1]/a')
            sht.cells(linha+1, 'A').add_hyperlink(address=item.get_attribute('href'),
                                                  text_to_display=item.text)
            linha += 1
    except Exception:
        pass
...
def navegacao(file: str):
    ...
    browser.find_element_by_xpath(
        '//*[@id="ctl00_contentPlaceHolderConteudo_BuscaNomeEmpresa1_btnTodas"]').click()
    ...
    if file == '':
        ...
        wkb.save(r'C:\Users\Gui Pupio\Desktop\Pessoal\python' + r'\base-' +
...
if __name__ == "__main__":
    ...
    baseFile = get_path_janela_dialogo(
